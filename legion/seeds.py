...
def validate_patch(patch: dict | None = None) -> list[str]:
    ...
    def collect(section: str) -> set[str]:
        ...
        for entry in p.get(section, []):
            key = entry.get("key")
            if not key:
                errors.append(f"{section}: entry missing 'key': {entry.get('name', entry)}")
                continue
            ...
            if str(entry.get("_status", "enabled")).lower() not in CONTENT_STATUS_ALIASES:
                errors.append(f"{section} '{key}': bad _status '{entry.get('_status')}'")
        return seen
    ...
    for m in p.get("materials", []):
        ...
        if m.get("stat_bonus_type") and m["stat_bonus_type"] not in valid_stats:
            errors.append(f"materials '{m.get('key')}': bad stat '{m['stat_bonus_type']}'")
        ...
        if kind == "potion" and not m.get("stat_bonus_value"):
            errors.append(
                f"materials '{m.get('key')}': potion needs stat_bonus_value"
            )
    _dummy_stats = {v: 10 for v in FORMULA_VARS}
    for s in p.get("active_skills", []):
        if s.get("effect_type") not in valid_effects:
            errors.append(f"active_skills '{s.get('key')}': bad effect '{s.get('effect_type')}'")
        try:
    ...
    for s in p.get("passive_skills", []):
        if s.get("stat_bonus_type") not in valid_stats:
            errors.append(f"passive_skills '{s.get('key')}': bad stat '{s.get('stat_bonus_type')}'")
        try:
    ...
    for w in p.get("weapons", []):
        ...
        for mount in w.get("passives", []):
            check("weapons", w["key"], "passive skill", mount.get("skill", ""), passives)
    ...
    for m in p.get("mobs", []):
        ...
        for ps in m.get("passives", []):
            ...
            if ps.get("requirement_type") and ps["requirement_type"] not in valid_reqs:
                errors.append(f"mobs '{m['key']}': bad requirement '{ps['requirement_type']}'")
    ...
    for r in p.get("recipes", []):
        ...
        if has_weapon == has_material:
            errors.append(f"recipes '{r.get('key')}': need exactly one of weapon/material")
        if has_weapon:
        ...
        if r.get("skill") and r["skill"] not in craft_skills:
            errors.append(f"recipes '{r.get('key')}': skill must be cook/brew (or absent for forge)")
        for i in r.get("inputs", []):
            check("recipes", r["key"], "input material", i.get("material", ""), materials)
...
async def pending_removals(patch: dict | None = None) -> dict[str, list[str]]:
    ...
    for section, model in _KEYED_SECTIONS:
        ...
        stale = await model.filter(key__isnull=False).exclude(
            key__in=list(patch_keys) or [""]
...
async def _load_sections(p: dict) -> int:
    ...
    for c in p.get("categories", []):
        await put(WeaponCategory, {"key": c["key"]}, {
            "name": c["name"], "status": _status_of(c),
        })
    categories = {c.key: c for c in await WeaponCategory.filter(key__isnull=False)}

    for s in p.get("active_skills", []):
        await put(ActiveSkill, {"key": s["key"]}, {
            "name": s["name"],
    ...
    for s in p.get("passive_skills", []):
        await put(PassiveSkill, {"key": s["key"]}, {
            "name": s["name"],
    ...
    for w in p.get("weapons", []):
        ...
        await sync_join(WeaponActiveSkill, "weapon_id", weapon.id, [
            ({"active_skill_id": actives[m["skill"]].id},
             {"tier": m.get("tier", 1), "mastery_level_required": m.get("req", 0)})
            for m in w.get("actives", [])
        ])
        await sync_join(WeaponPassiveSkill, "weapon_id", weapon.id, [
    ...
    for g in p.get("grounds", []):
        await sync_join(GroundMob, "ground_id", grounds[g["key"]].id, [
            ({"mob_id": mobs[e["mob"]].id}, {"weight": e.get("weight", 1)})
            for e in g.get("pool", [])
        ])

    for s in p.get("sites", []):
        await put(GatherSite, {"key": s["key"]}, {
            "name": s["name"],
            "skill": s["skill"],
    ...
    for s in p.get("sites", []):
        await sync_join(SiteYield, "site_id", sites[s["key"]].id, [
            ({"material_id": materials[y["material"]].id},
             {"weight": y.get("weight", 1),
              "min_qty": y.get("min", 1), "max_qty": y.get("max", 1)})
            for y in s.get("yields", [])
        ])

    for r in p.get("recipes", []):
        await put(Recipe, {"key": r["key"]}, {
            "name": r["name"],
            "skill": r.get("skill"),
    ...
    for r in p.get("recipes", []):
        await sync_join(RecipeMaterial, "recipe_id", recipes[r["key"]].id, [
    ...
    for c in p.get("upgrade_costs", []):
        obj, was_created = await LegionUpgradeCost.update_or_create(
            level=c["level"], material_id=materials[c["material"]].id,
